  ...
  scratchCards.forEach((scratchCard) => {
    ...
    const startAutoScratch = () => {
      ...
      const animateScratch = () => {
        ...
        // Random scratching pattern - smaller and fewer scratches per frame
        const numScratches = 5; // Reduced from 50 to 5
        for (let i = 0; i < numScratches; i++) {
          const x = Math.random() * 400;
          const y = Math.random() * 300;
          const radius = 12 + Math.random() * 12; // Reduced from 20-50 to 8-20
